...
- **Google Analytics 4** — GA4 script loaded via `next/script` (`afterInteractive`) in the root layout. SPA route changes tracked via `GARouteTracker` (client component, wrapped in Suspense). Custom events fired from `WarrantyForm`: `warranty_form_start`, `warranty_form_submit_attempt`, `warranty_form_validation_error`, `warranty_file_upload_start`, `warranty_file_upload_complete`, `warranty_submitted`, `warranty_submit_error`, `warranty_rate_limited`. Inactive if `NEXT_PUBLIC_GA_MEASUREMENT_ID` is unset. Var is baked into the JS bundle at build time — `build.bat` passes it as a Docker `--build-arg` from `.env`.
